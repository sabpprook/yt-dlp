...
- `744`: _`warn_if_short_id(self, argv)`_

- `760`: `add_info_extractor(self, ie)`
    - `self`: use proxy ydl object if needed
        - Seems like we can work around this for now

- `768`: _`get_info_extractor(self, ie_key)`_

- `780`: `self.add_default_info_extractors()`
    - `allowed_extractors`

- `798`: `add_post_processor(self, pp, when='post_process')`
    - `self._pps`

- `804`: _`add_post_hook(self, ph)`_

- `808`: _`add_progress_hook(self, ph)`_

- `812`: `add_postprocessor_hook(self, ph)`
    - `self._pps`

- `819`: _`_bidi_workaround(self, message)`_

- `832`: _`_write_string(self, message, out=None, only_once=False)`_
    - `encoding`

- `839`: _`to_stdout(self, message, skip_eol=False, quiet=None)`_

- `849`: _`to_screen(self, message, skip_eol=False, quiet=None)`_

- `860`: _`to_stderr(self, message, only_once=False)`_

- `868`: _`_send_console_code(self, code)`_

- `873`: _`to_console_title(self, message)`_
...
- `1099`: `prepare_outtmpl(self, outtmpl, info_dict, sanitize=False)`
    ...
    - Potentially get extractor from `info_dict`, match for url or sth like
...
- `1875`: _`__process_iterable_entry(self, entry, download, extra_info)`_

- `1879`: _`_build_format_filter(self, filter_spec)`_
...
- `2382`: _`_fill_common_fields(self, info_dict, is_video=True)`_

- `2436`: _`_raise_pending_errors(self, info)`_
...
## `class InfoExtractor`
- `728`: `cookiejar(self)`
    - `self._downloader.cookiejar`
- `773`: `_request_webpage(self, url_or_request, video_id, note=None, errnote=None, fatal=True, data=None, headers=None, query=None, expected_status=None)`
    - `self._downloader._first_webpage_request`
...
- `1104`: `def get_param(self, name, default=None, *args, **kwargs)`
    - `self._downloader.params`
        - Rewrite as:
            ...
            return default
            ```
        - `if self.IE_NAME in self.params.get(`
...
- `YoutubeDL.py:877`: If console sequences are supported, it should use these instead on windows.
...
- `extractor/dtube.py`: `self._downloader._opener.open(video_url, timeout=5).close()`
- `extractor/abematv.py:207-208`: `remove_opener(self._downloader, ...`
